...
export default function EditDealPage() {
  ...
  // Load firms, details, and settings
  useEffect(() => {
    async function loadData() {
      try {
        const [firmsRes, dealRes, settingsRes] = await Promise.all([
        ...
        if (firmsRes.ok && dealRes.ok) {
          ...
          setFormData({
            ...
            tag: dealData.tag || (dealData.deal_type === 'bogo' ? 'BOGO' : dealData.deal_type === 'best_value' ? 'BEST OFFER' : dealData.discount_label || ''),
            description: dealData.description || '',
            firm_id: dealData.firm_id || '',
            is_featured: !!dealData.is_featured,
            is_bogo: !!dealData.is_bogo || dealData.deal_type === 'bogo',
            is_best_offer: !!dealData.is_best_offer || dealData.deal_type === 'best_value',
            is_main_offer: !!dealData.is_main_offer,
            priority: dealData.priority !== undefined ? Number(dealData.priority) : 0,
            discord_code: dealData.discord_code || '',
            affiliate_url: dealData.affiliate_url || '',
            logo_url: dealData.logo_url || '',
            expires_at: dateVal,
            status: dealData.status || 'active',
            deal_type: dealData.deal_type || 'best_value',
          })
        }
        if (settingsRes.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err)
      setError('Network error updating deal')
    } finally {
      setSaving(false)
    }
  }
  ...
  if (loading) {
    return <div className="text-center py-20 text-text-secondary">Loading promo deal...</div>
  }
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-4xl font-extrabold tracking-tight text-text-primary mb-2 afx-gradient-heading">
            Edit Promo Code / Offer
          </h1>
          <p className="text-text-secondary text-sm">Update coupon codes, discount labels, display tags, and details.</p>
        </div>
      </div>
