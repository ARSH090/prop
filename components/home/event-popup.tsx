...
export function EventPopup() {
  ...
  useEffect(() => {
    // Show popup on first open after a 2-second delay
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fade-in">
      <div className="relative w-full max-w-md bg-[#0D1321] border border-border-subtle rounded-3xl overflow-hidden shadow-2xl neon-border-cyan animate-slide-up">
        ...
        <div className="p-6 space-y-4">
          <div className="space-y-1">
            <h3 className="text-xl font-black text-text-primary afx-gradient-heading">
              ANURAJ FX Trading Tournament Q3 2026
            </h3>
          ...
          <div className="flex flex-wrap gap-4 py-3 border-y border-border-subtle/50 text-xs text-text-secondary">
            <div className="flex items-center gap-1.5 font-bold">
              ...
              <span>August 15–29, 2026</span>
            </div>
            <div className="flex items-center gap-1.5 font-bold">
              <Trophy className="w-3.5 h-3.5 text-accent-purple" />
              <span className="text-accent-purple">$10,000 Prize Pool</span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            <button
              ...
              className="px-5 py-2.5 rounded-xl text-xs font-bold text-text-secondary hover:text-text-primary transition-colors bg-transparent border border-border-subtle"
            >
              Skip
            </button>
            <Link
        ...
        <button
          onClick={() => setIsOpen(false)}
          className="absolute top-4 right-4 p-1.5 rounded-full bg-black/60 hover:bg-black text-white hover:text-accent-cyan transition-colors"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
